...
delta.fit.tab <- function(data, title, more.footnote = NULL){
  ...
  for (i in 2:nrow(data)){
    data$'ΔChi-square(p)' <- rep(NA, nrow(data))
    ...
    data$'ΔChi-square(p)' = paste0(diff, "(", diff.p, ")")
  }
...
### Constrain 20 common factor loadings equal step by step(inv2.model)
...
itemset1 <- "ITEM2 + ITEM3 + ITEM6 + ITEM8 + ITEM13 + ITEM14 + ITEM16 + ITEM20 + ITEM12"
itemset2 <- "ITEM10 + ITEM11 + ITEM15 + ITEM22"
itemset3 <- "ITEM7 + ITEM9 + ITEM12 + ITEM17 + ITEM18 + ITEM19 + ITEM21"
...
### Constrain 20 common factor loadings equal step by step(inv3.model)
...
```
 ...
 The fit statistics of inv3.model: MLM χ2[420] 969.990, CFI 0.937, RMSEA 0.045, SRMR 0.054.
 
```{r}
delta.fit.tab(fit.indices.123, "title")
```
