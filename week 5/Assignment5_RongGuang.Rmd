...
The texts that reflect my understanding/questions/doubts have been highlighted in \textcolor{red}{red color}.
The texts that describes important steps/results or that corresponds to certain exercise requirement have been highlighted in \textcolor{blue}{blue color}.
...
\textcolor{blue}{See table 7. In reviewing this information for elementary teachers, we observe two MIs larger than all other MIs (ITEM7 with ITEM4; ITEM19 with ITEM18); both represent residual co-variances. I followed Byrne's step in addressing these parameters (free the first relationship between ITEMs 7 and 4).} According to Byrne, of the two, only the residual covariance between Items 7 and 4 is substantively viable in that there is a clear overlapping of item content. In contrast, the content of Items 19 and 18 exhibits no such redundancy, and, thus, there is no reasonable justification for including this parameter in a succeeding Model 3.
...
title(main = list("Secondary School Teachers",
                  cex = 1, font =1), outer = F, line = -3)
mtext("Figure 1 Baseline MBI models for two groups of teacher",
...
\textcolor{blue}{See table 13. Of all the eligible parameters, the factor loading of Item 11 on DP appears to be the most problematic in terms of its equivalence across elementary and secondary teachers. I relaxed this factor loading(Item 11 by DP) from constraint for establishing the the next model, model3 (inv3.model)}

(3) Re-specify model2 to fit model 3 (inv3.model) by relaxing a parameter

   \textcolor{blue}{Equality constraint on Parameter "DP by ITEM11" was relaxed. }
...
inv3.fit |> 
  ...
  kable(
    digits = 3,
    booktabs = T,
    caption = "Re-estimates for relaxed parameter 'DP =~ ITEM11'"
  ) |> 
  kable_styling () |> 
...
inv4.fit |> 
  ...
  kable(
    digits = 3,
    booktabs = T,
    caption = "Re-estimates for relaxed parameter 'DP by ITEM15'"
  ) |> 
  kable_styling () |> 
  column_spec(1, width = "6.5cm") |> 
  column_spec(2, width = "1.8cm", color = "red") |> 
...
See table 17. Inv4.model produced these results: MLM chi-square[419] 961.653, CFI 0.938, RMSEA 0.045, SRMR 0.054. \textcolor{blue}{Comparison of this model with the configural model (see table 17, 3rd column) yielded a corrected MLM chi-square difference of 20.964 (p = 0.281), which is n.s.} A review of the estimated parameters revealed a fairly substantial difference for the specified parameter (0.684 for elementary and 0.963 for secondary teachers). See table 18.

### sub-section summary

All items on the MBI, except for items 11 and 15, both of which load on Factor2, are operating equivalently across the two groups of teachers.
...
In defining the model with code, the co-variance of the pair of items get distinct coefficient letter, and this is to ensure that each of these parameters will be constrained equal across the two groups and NOT to each other.
...
key.table3 <- delta.fit.tab(fit.indices.45, 
              "Fit indices for the Inv5.model and Inv4.model",
...
Specified residual co-variances between Items 6 and 16, Items 1 and 2, and Items 10 and 11 are operating equivalently across elementary and secondary teachers.
...
#print the table
key.table4 <- delta.fit.tab(fit.indices.56, 
              "Fit indices for the Inv6.model and Inv5.model",
...
\textcolor{red}{Here I got a MLM chi-square difference of 12.941. Though it only deviated from the value on slides (12.117), the samll difference made p value fell significant (p = 0.044, vs p= 0.059 on slides). I hope this is due to the difference in algorithmns. However, if this were true, we would be surprised by how different tools leads to different conclusions in the areas using SEM.}

### Section summary

Factor variances and co-variances are operating equivalently across elementary and secondary teachers.
...
&nbsp;&nbsp;&nbsp;(5) Use the last model we established in last step (I will call it COV reference model) and, this time, further constrain equal across groups all the factor variances and co-variances and fit it. Comparing this model to COV reference model, if the MLM chi-square difference yields non-significance, we can conclude that the multi-group in-variance assumption also holds for factor variances and co-variances. \textcolor{red}{See table 23 below: factor-variance/covariance-constrained model (lower) is compared to COV reference model (upper).}

If the comparison yields significant result, 
