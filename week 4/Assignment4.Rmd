...
The estimation of Model 6 yielded a MLM χ2(422) value of 1297.489 (scaling correction factor = 1.116), with the remaining fit indices as follows: CFI = 0.957, TLI = 0.949, RMSEA = 0.038, and SRMR = 0.039. As might be expected given the newly specified path involving Factor 10, the difference in fit between this model (Model 6) and its predecessor (Model 5) was found to be statistically significant (MLM∆χ2[1] = 23.358). See tab 17.
...
The estimation of Model 7 yielded a MLM χ2(421) value of 1267.208 (scaling correction factor = 1.116), with the remaining fit indices as follows: CFI = 0.958, TLI = 0.951, RMSEA = 0.038, and SRMR = 0.038. Again, the difference in fit between this model (Model 7) and its predecessor (Model 6) was found to be statistically significant (MLM∆χ2[1] = 23.057).

Here again, the new parameter added to the model (F11 on F8) was found to be statistically significant (Estimate = -0.42; Z-statistics = -4.688). However, the structural regression paths F8 (Self-Esteem) on F7 (Peer Support) and F12 (Personal Accomplishment) on F10 (Emotional exhaustion) remained non-signification.

At this point, I believed that we had exhausted the search for a better fitting model that best represents the data for secondary teachers. Thus, I started to delete regression path that were originally specified in the hypothesized model but remain statistically non-significant as per the results for Model 7. Accordingly, Model 8 was specified in which the two non-significant parameters noted above (F7 → F8; F10 → F12) are not included. Note that one of the excluded paths was different from Byrne's decision, where she deleted F12 on F1 instead of F12 on F10. See next section.
...
```{r}
sem.measure8 <- fitMeasures(sem8,    #obtain specified measured.
                            c("chisq.scaled",
                              "df.scaled",
                              "pvalue.scaled",
                              "cfi.scaled",
  ...
  round(3)
#combine the 2 sets of indices and save into a new object
sem.compare8 <- rbind(sem.measure1, sem.measure2,
                      sem.measure3, sem.measure4, 
                      sem.measure5, sem.measure6,
                      sem.measure7, sem.measure8) |>
  data.frame()
sem.compare8.tab <- sem.compare8

#add column "chi square" and place values into it
sem.compare8.tab$"ΔChi-square" <- rep(NA, 8)
sem.compare8.tab$"ΔChi-square"[2] <- chi.diff(sem1, sem2)#calculate chisq difference
sem.compare8.tab$"ΔChi-square"[3] <- chi.diff(sem2, sem3)
sem.compare8.tab$"ΔChi-square"[4] <- chi.diff(sem3, sem4)
sem.compare8.tab$"ΔChi-square"[5] <- chi.diff(sem4, sem5)
sem.compare8.tab$"ΔChi-square"[6] <- chi.diff(sem5, sem6)
sem.compare8.tab$"ΔChi-square"[7] <- chi.diff(sem6, sem7)
sem.compare8.tab$"ΔChi-square"[8] <- chi.diff(sem7, sem8)

#turn named vector to data frame
sem.compare8.tab<- sem.compare8.tab %>%
  select("Chi-square" = chisq.scaled, "DF"=df.scaled,
         "p value"=pvalue.scaled, "ΔChi-square", "CFI"= cfi.scaled,
         "TLI"= tli.scaled, "RMSEA"= rmsea.scaled,
         "SRMR"=srmr_bentler, "CSF"=chisq.scaling.factor)
...
#There is no chisq-diff value for the first model, so place a "--" in the cell
sem.compare8.tab[1,4] <- "--"

#add row names
rownames(sem.compare8.tab) <- c("model1", "model2*",
                                "Model3†", "Model4‡",
                                "Model5§", "Model6¶",
                                "Model7**", "Model8")
