...
For the sake of plotting, I deleted the F7 with two of its indicators from the model (which I shouldn't, since they still have a position in the measurement model, but I haven't found a better way to hide a node using semplot). However, this is only for demonstrating the structure of final model. It won't cause any misunderstanding, I think.
...
```{r}
saveRDS(model.table, file = "modeltable20.rds")
...
```{r}
a |>   na.omit() |> 
  select(everything(), train = 'train/test') |> 
  ggplot(aes(x = factor(modification.n), 
             y = mean.chi.square.diff,  
             color = train, group = train)) +
    geom_line()+
  geom_point()+
  geom_ribbon(aes(ymin = 
                    mean.chi.square.diff-1.96*sd.chi.square.diff/sqrt(n.chi.square.diff), 
                  ymax = 
                    mean.chi.square.diff+1.96*sd.chi.square.diff/sqrt(n.chi.square.diff),  
                  fill = train), alpha =0.25,
...
a |>   na.omit() |> 
  ...
  geom_point()+
  geom_ribbon(aes(ymin = 
                    mean.tli-1.96*sd.tli/sqrt(n.tli), 
                  ymax = 
                    mean.tli+1.96*sd.tli/sqrt(n.tli),  
                  fill = train), alpha =0.25,
                position = position_dodge(0.05))
...
Write a function that automate the judicious selection of regression path to set free. 

```{r}
find.path <- function(mi.train){
  interest <- c("F11DP", "F10EE", "F12PA")
  for(r1 in 1:nrow(mi.train)){
    lhsterm <- mi.train[r1,1]
    rhsterm <- mi.train[r1,3]
    epcterm <- mi.train[r1,5]
    ...
    if (lhsterm %in% interest & epcterm*value.left*value.right>0){
      ...
      for(r2 in 1:nrow(mi.train)){
        lhsterm <- mi.train[r2,1]
        rhsterm <- mi.train[r2,3]
        epcterm <- mi.train[r2,5]
        value.left <- logic[lhsterm]
        value.right <- logic[rhsterm]
        free.parameter.temp <- paste(lhsterm, "~", rhsterm)
        if(epcterm*value.left*value.right>0){
          break} 
        }
      free.parameter.final <- free.parameter.temp
    } 
  print(free.parameter.final)
}
 
find.path(mi.train)
free.parameter.final
...
test <- c("a", "b", "c")
"a" %in% test
    lhsterm <- mi.train[1,1]
    rhsterm <- mi.train[1,3]
    epcterm <- mi.train[1,5]
    
value.left <- logic[lhsterm]
    value.right <- logic[colnames(logic) == rhsterm]
   
  lhsterm  
  logic
logic[rhsterm]   
    eval(parse(text=sem[i]))
    logic
```

```{r}
sem <- sem(model, data = mbi, estimator = "MLM", mimic = "Mplus")

mi.train <- modindices(sem,standardized = TRUE, sort. = TRUE) |> 
  filter(op == "~") |>  
  filter (!rhs %in% c("F8SELF","F9ELC","F10EE","F11DP","F12PA"))  
mi.train <- mi.train[-2,]
mi.train
