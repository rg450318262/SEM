...
a <- mbi |> 
  ...
  labs(title = 
         'fig2 (a). Distribution of parcelled scores from BMI instrument') +
  labs(x = "Parcelled scores", y = "Parcelled items") +
  theme_ipsum() +
    theme(
...
b <- mbi |> 
  ...
  scale_fill_viridis(name = "parcelled score", option = "C") +
  labs(title = 
         'fig2 (b). Distribution of parcelled scores from TSS instrument') +
  labs(x = "Parcelled scores", y = "Parcelled items")+
  theme_ipsum() +
    theme(
      legend.position="none",
...
semPaths(semPlotModel(initial_model),
         ...
         optimizeLatRes = T,
         exoVar = F)
title(main = list("Figure 1. Hypothesized model of teacher burnout", cex = 1.5, font =1),
     outer = F, line = -2)
title(sub = "Notes: Red arrow indicates factor residuals; gray arrow indicates error residuals; 
     blue arrow indicates regression path; black arrow indicates factor loading", line = 0, adj = 0.7)
```
...
sem1 <- sem(model1, data = mbi, estimator = "MLM")
